export default class PhotographerList { // Créer une class "PhotographerList"
  constructor(selector, photographers) { // Créer le constructor avec comme paramètre "selector" et "photographers"
  ...
  getRelevantPhotographers(filterTags) {
    ...
    const filteredPhotographersArray = [];
  ...
  displayRelevantCards(selectedTags) {
    ...
    if (!(selectedTags === undefined)) {
      ...
      let relevantPhotographersArraysJoined = [].concat([], ...relevantPhotographers);
      // doublons supprimés
  ...
  // Créer les éléments HTML et ajouter la description des profils + tags
  ...
  createCard(photographer) {
    ...
    portrait.src = `resources/profil/${photographer.portrait}`;
  ...
  getTagContent() {
    ...
    const activeTagsContent = Array.from(activeTags).map((activeTag) => activeTag.dataset.tagName);
